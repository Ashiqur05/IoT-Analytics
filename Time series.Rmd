...
#more period i am using more data point i will going to lose
```
In this data set simple moving average is kind of a winner for me because with that i am able to see very clearly there is a trand that is present,there is some cyclic or random or seasonal that is present but with this Exponential Moving Average i could only see that probly there is a trand but there is a irregular component which is present in this time series.
## Decomposition
...
df_arima<-(housePWR_mnthTS[,3]) 
plot(df_arima)
cycle(df_arima)
#Box plot 
boxplot(df_arima ~ cycle(df_arima))#Box plot across months will give us a sense on seasonal effect
...
plot(diff(log(df_arima)))

df_fit_arima_mnth_TS_auto<-auto.arima(df_arima)
...
acf(df_arima) # determine the value of q=2
#acf(diff(log(df_arima)))

pacf(df_arima)  # determine the value of p=0
df_fit_arima_mnth_TS<-arima(df_arima, c(0,0,2))

df_fit_arima_mnth_TS_pred<-predict(df_fit_arima_mnth_TS,n.ahead = 1*11)

ts.plot(df_arima,df_fit_arima_mnth_TS_pred)
